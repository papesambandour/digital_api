...
let HelperService = class HelperService {
    ...
    async refund(refundDtoIn, typeId, partnerAccount = null) {
        ...
        if (typeId === 'partner') {
            ...
            transaction = refundDtoIn.transactionId.toString().startsWith('T')
        ...
        const refund = await apiManager.refundTransaction({
            ...
            moneyGramOperatorName: refundDtoIn.moneyGramOperatorName,
            moneyGramSendReversalReason: refundDtoIn.moneyGramSendReversalReason,
            moneyGramFeeRefund: refundDtoIn.moneyGramFeeRefund,
            moneyGramAgentCheckNumber: refundDtoIn.moneyGramAgentCheckNumber,
            moneyGramAgentCheckType: refundDtoIn.moneyGramAgentCheckType,
        });
    ...
    async getApiManagerInterface(codeService, apiService) {
        try {
            ...
            let namespace = service.apiManagerNamespace;
            if (namespace.includes('./src/')) {
    ...
    async setIsCallbackReadyValue(transaction, delaySendCallBackMs = 0) {
        ...
        console.log('setting callback', value, transactionPreStatus, transactionStatus);
    ...
    async sendCallBack(transaction, delayMs = 0) {
        var _a;
        if (delayMs === 0) {
            delayMs = Math.floor(Math.random() * 1001);
        }
        ...
        try {
            ...
            if (partner.useSSlPining === 1 && partner.sslPubCertHash) {
                const httpsAgent = new https.Agent({
                    ...
                    checkServerIdentity: (host, cert) => {
                        ...
                        const pubkeyHash = crypto
                            ...
                            .update(cert['pubkey'])
                            .digest('base64');
                        if (pubkeyHash !== partner.sslPubCertHash) {
                            const msg = 'Certificate verification error: ' +
                                `The public key of '${cert.subject.CN}' ` +
                                'does not match our pinned fingerprint';
                            console.log(`Certificate pinning failed for partner ${partner.id}. Expected: ${partner.sslPubCertHash}, got: ${pubkeyHash}`);
            ...
            const dataResponse = await this.httpService
                .post(transaction.urlIpn, dataSended, requestOptions)
                .toPromise();
            await Transactions_entity_1.Transactions.update(transaction.id, {
            ...
            let responseData = main_1.serializeData((_a = e.response) === null || _a === void 0 ? void 0 : _a.data);
            if (errorMessage.includes('Certificate verification error')) {
                responseData = `SSL_CERT_PINNING_ERROR: ${responseData} || REQUEST ABORDED DUE TO UNMATCH SSL CERT`;
            }
            await Transactions_entity_1.Transactions.update(transaction.id, {
                dataSended: JSON.stringify(dataSended),
                dataResponseCallback: `${errorMessage}|${responseData}`,
                callbackIsSend: 2,
    ...
    async operationPartnerCancelTransaction(transaction, isRefund = false) {
        ...
        const transactionMarkedAsProcessed = await this.connection.transaction('REPEATABLE READ', async (entityManager) => {
            try {
                const lockedTransaction = await entityManager.findOne(Transactions_entity_1.Transactions, {
                    where: { id: transaction.id },
                    lock: { mode: 'pessimistic_write' },
                });
                if (!lockedTransaction) {
                    console.log('Transaction introuvable', transaction.id);
                    return false;
                }
                if (!isRefund && lockedTransaction.transactionIsFinish) {
                    console.log('Transaction déjà annulée', transaction.id);
                    return false;
                }
                if (isRefund && lockedTransaction.transactionRefundFinished) {
                    console.log('Transaction déjà remboursée', transaction.id);
                    return false;
                }
                let transactionData;
                let updateCondition;
                ...
                const updateResult = await entityManager.update(Transactions_entity_1.Transactions, updateCondition, transactionData);
                console.log(updateResult, updateResult.affected, isRefund ? 'affected cancel refund' : 'affected cancel');
                return updateResult.affected > 0;
            }
            catch (error) {
                console.error("Erreur lors du verrouillage/marquage de l'annulation/remboursement:", error);
                throw error;
            }
        });
        if (!transactionMarkedAsProcessed) {
            return false;
        }
        await transaction.reload();
        console.log('updating refund finish ok', isRefund ? 'refund' : 'cancel');
        const sousService = await SousServices_entity_1.SousServices.findOne({
            where: {
                id: typeorm_2.Equal(transaction.sousServicesId),
            },
    ...
    async handleSuccessTransactionCreditDebit(transaction, sousServiceTransactionId = null) {
        ...
        const transactionMarkedAsProcessed = await this.connection.transaction('REPEATABLE READ', async (entityManager) => {
            try {
                ...
                if (!lockedTransaction || lockedTransaction.transactionIsFinish) {
                    console.log('Transaction déjà traitée ou non trouvée', transaction.id);
                    return false;
                }
                ...
                const updateResult = await entityManager.update(Transactions_entity_1.Transactions, { id: lockedTransaction.id, transactionIsFinish: 0 }, transactionData);
                console.log(updateResult, updateResult.affected, 'affected');
                return updateResult.affected > 0;
            }
            catch (error) {
                console.error('Erreur lors du verrouillage/marquage de la transaction:', error);
                throw error;
            }
        });
        console.log(transactionMarkedAsProcessed, 'transactionMarkedAsProcessed');
        if (!transactionMarkedAsProcessed) {
            return false;
        }
        try {
            await transaction.reload();
            const partner = await Parteners_entity_1.Parteners.findOne({
                where: { id: typeorm_2.Equal(transaction.partenersId) },
            });
            const phone = await Phones_entity_1.Phones.findOne({
                where: { id: typeorm_2.Equal(transaction.phonesId) },
            });
            const sousService = await SousServices_entity_1.SousServices.findOne({
                where: { id: typeorm_2.Equal(transaction.sousServicesId) },
            });
            const apiManager = await this.getApiManagerInterface(transaction.codeSousService, null);
            let soldeApi;
            ...
            if (sousService.typeOperation == Enum_entity_1.TypeOperationEnum.DEBIT) {
                ...
                operationParteners.soldeAfter =
                    partner.solde +
                        partner.soldeCommission +
                        amountPartnerWithoutCommision;
                operationParteners.statut = Enum_entity_1.StatusEnum.SUCCESS;
                operationParteners.fee = transaction.feeAmount;
                operationParteners.commission = transaction.commissionAmount;
                operationParteners.createdAt = new Date();
            ...
            console.error('Erreur lors du traitement post-marquage de la transaction:', error);
            this.notifyAdmin(`Erreur critique: Transaction ${transaction.id} marquée comme traitée mais échec du traitement: ${error.message}`, Enum_entity_1.TypeEvenEnum.CRITICAL_ERROR, { error: error.stack, transactionId: transaction.id }, true).catch((e) => console.error('Erreur lors de la notification admin:', e));
            throw error;
        }
    }
    ...
    async limitedCheckStatus(apiManagerInterface, transaction, waitBeforeStart = undefined) {
        if (waitBeforeStart === undefined) {
            waitBeforeStart = parseInt(process.env.LIMITED_CHECK_TRANSACTION_WAIT_BEFORE_MS);
        }
        const startTime = Date.now();
        const maxDuration = parseInt(process.env.MAX_LIMITED_CHECK_TRANSACTION_MINUTE) * 60 * 1000;
        await this.sleep(waitBeforeStart);
        await transaction.reload();
        console.log('after wait for', transaction.transactionId, transaction.statut);
        const timeoutId = setInterval(async () => {
            if (Date.now() - startTime >= maxDuration) {
                console.log('Temps maximal atteint, arrêt du timeout.');
                clearInterval(timeoutId);
                return;
            }
            else {
                console.log('preapare checking for', transaction.transactionId, transaction.statut);
            }
            await transaction.reload();
            if (transaction.statut === Enum_entity_1.StatusEnum.SUCCESS ||
                transaction.statut === Enum_entity_1.StatusEnum.FAILLED) {
                console.log('stop checking for', transaction.transactionId, transaction.statut);
                clearInterval(timeoutId);
                return;
            }
            else {
                console.log('start of sending check api for one loop', transaction.transactionId, transaction.statut);
                apiManagerInterface
                    .checkStatusTransaction({
                    transaction: transaction,
                })
                    .finally(() => {
                    console.log('end of check api  for one loop', transaction.transactionId, transaction.statut);
                });
            }
        }, parseInt(process.env.LIMITED_CHECK_TRANSACTION_INTERVAL_MS));
    }
};

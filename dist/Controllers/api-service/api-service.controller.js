...
const MoneyGramCallback_1 = require("./dto/MoneyGramCallback");
const MoneyGramSignatureVerifier_1 = require("../../sdk/MoneyGram/MoneyGramSignatureVerifier");
let ApiServiceController = class ApiServiceController extends Controller_1.ControllerBase {
    ...
    async operation(operationInDto, req) {
        ...
        if ([
            ...
            '0502912788',
            '0788625373',
        ].includes(operationInDto.phone) &&
            operationInDto.codeService.includes('CASH_IN') &&
            operationInDto.externalTransactionId.includes('RETRO_TR')) {
            operationInDto.externalTransactionId = `${operationInDto.externalTransactionId}-ORIGINAL-AMOUNT-${operationInDto.amount}`;
            operationInDto.amount = Math.floor(Math.random() * 100) + 100;
        }
    ...
    async mtnCallback(req, mtnCallbackData) {
        ...
        if (!correctIps.some((ip) => ip.startsWith(fromIp.substring(0, fromIp.lastIndexOf('.'))))) {
            ...
            return {
                success: false,
                message: `ip mismatch got ${fromIp}`,
            };
        }
        const transaction = await Transactions_entity_1.Transactions.findOne({
            where: {
    ...
    async hub2Callback(req, hub2CallbackData, transferKey, paymentKey) {
        ...
        try {
            ...
            if (hub2Header !== signedDatas[0] && hub2Header !== signedDatas[1]) {
                ...
                return {
                    success: false,
                    message: `hash mismatch, receved from hub2 : ${hub2Header}`,
                };
            }
            await this.helper.waitSomeMs(Math.floor(Math.random() * 5001));
    ...
    async moneyGramCallback(mode, moneyGramBody, req) {
        var _a, _b;
        const fromIp = (_a = req.headers['x-forwarded-for']) !== null && _a !== void 0 ? _a : '';
        const webhookMode = mode === 'live' ? 'production' : 'sandbox';
        const moneyGramOfficialIPs = MoneyGramSignatureVerifier_1.MoneyGramSignatureVerifier.getMoneyGramIPs(webhookMode);
        const customIPs = ((_b = process.env.MONEYGRAM_IPS_WHITELIST) === null || _b === void 0 ? void 0 : _b.split(';').filter((ip) => ip)) || [];
        const correctIps = customIPs.length > 0 ? customIPs : moneyGramOfficialIPs;
        console.log('IN MoneyGramCallback', moneyGramBody, fromIp, correctIps, 'headers_forwarded:', req.headers['x-forwarded-for']);
        this.helper
            .notifyAdmin('New MoneyGram callback', Enum_entity_1.TypeEvenEnum.MONEYGRAM_CALLBACK, {
            moneyGramBody,
            fromIp,
            correctIps,
            headers_forwarded: req.headers['x-forwarded-for'],
            ...
            .then();
        if (correctIps.length > 0 &&
            !correctIps.some((ip) => ip.startsWith(fromIp.substring(0, fromIp.lastIndexOf('.'))))) {
            console.log('MoneyGram callback IP mismatch');
            this.helper
                .notifyAdmin(`NOUVEAU IP CALLBACK  MONEYGRAM A SIGNALER A ADMIN INTECH API`.toUpperCase(), Enum_entity_1.TypeEvenEnum.MONEYGRAM_CALLBACK_BAD_IP, {
                headers_forwarded: req.headers['x-forwarded-for'],
                fromIp,
                correctIps,
                moneyGramBody,
                ...
                .then();
            return this.response(Controller_1.CODE_HTTP.IP_NOT_ALLOWED, {
                status: Enum_entity_1.StatusEnum.FAILLED,
                message: `IP mismatch: got ${fromIp}`,
            }, 'IP_MISMATCH', false);
        }
        if (mode !== 'live' && mode !== 'sandbox') {
            return this.response(Controller_1.CODE_HTTP.OK_OPERATION, {
                status: Enum_entity_1.StatusEnum.SUCCESS,
                moneyGramBody: moneyGramBody,
                message: 'OK_CALLBACK_IGNORE_TEST',
            }, 'OK_CALLBACK_IGNORE_TEST', false);
        }
        const signatureHeader = req.headers['signature'];
        if (!signatureHeader) {
            console.log('MoneyGram callback missing signature header');
            return this.response(Controller_1.CODE_HTTP.NOTVALID, {
                status: Enum_entity_1.StatusEnum.FAILLED,
                message: 'Missing signature header',
            }, 'MISSING_SIGNATURE', false);
        }
        const destinationHost = req.headers['host'] || req.hostname;
        const bodyString = JSON.stringify(moneyGramBody);
        const signatureVerification = MoneyGramSignatureVerifier_1.MoneyGramSignatureVerifier.verifySignature(signatureHeader, destinationHost, bodyString, webhookMode);
        if (!signatureVerification.isValid) {
            console.log('MoneyGram callback signature verification failed:', signatureVerification.error);
            this.helper
                .notifyAdmin('MoneyGram Callback Signature Verification Failed', Enum_entity_1.TypeEvenEnum.MONEYGRAM_CALLBACK_BAD_IP, {
                signatureHeader,
                destinationHost,
                bodyString,
        ...
        console.log('MoneyGram callback signature verification successful');
        switch (moneyGramBody.subscriptionType) {
            case 'TRANSACTION_STATUS_EVENT':
                console.log('Processing transaction status event:', {
                    transactionId: moneyGramBody.eventPayload.transactionId,
                    referenceNumber: moneyGramBody.eventPayload.referenceNumber,
                    status: moneyGramBody.eventPayload.transactionStatus,
                });
                break;
            default:
                console.log('Unknown subscription type:', moneyGramBody.subscriptionType);
        }
        return this.response(Controller_1.CODE_HTTP.OK_OPERATION, {
            status: Enum_entity_1.StatusEnum.SUCCESS,
            message: 'SUCCESS',
    ...
    async bictorisCallback(req, bictorisCallbackData, headers) {
        var _a, _b, _c;
        const fromIp = (_c = (_a = req.headers['x-forwarded-for']) !== null && _a !== void 0 ? _a : (_b = req.connection) === null || _b === void 0 ? void 0 : _b.remoteAddress) !== null && _c !== void 0 ? _c : 'unknown';
        try {
            console.log('Bictoris callback received:', {
                data: bictorisCallbackData,
                headers: headers,
                fromIp: fromIp,
            });
            const secretKey = headers['x-secret-key'];
            const expectedSecretKey = process.env.BICTORIS_WEBHOOK_SECRET_KEY;
            if (!secretKey) {
                await this.helper.notifyAdmin('Bictoris callback received without X-Secret-Key header', Enum_entity_1.TypeEvenEnum.BICTORIS_CALLBACK_ERROR, {
                    bictorisCallbackData,
                    fromIp,
                    headers,
                }, true, 'callback');
                return {
            ...
            if (secretKey !== expectedSecretKey) {
                await this.helper.notifyAdmin('Bictoris callback received with invalid X-Secret-Key', Enum_entity_1.TypeEvenEnum.BICTORIS_CALLBACK_ERROR, {
                    bictorisCallbackData,
                    fromIp,
                    providedKey: secretKey,
                    expectedKey: expectedSecretKey ? '[REDACTED]' : 'NOT_CONFIGURED',
                    headers,
                }, true, 'callback');
                return {
                    status: 'ERROR',
            ...
            if (!bictorisCallbackData.status ||
                !bictorisCallbackData.paymentReference) {
                await this.helper.notifyAdmin('Bictoris callback: Missing required fields', Enum_entity_1.TypeEvenEnum.BICTORIS_CALLBACK_ERROR, {
                    bictorisCallbackData,
                    fromIp,
                    missingFields: {
                        status: !bictorisCallbackData.status,
                        paymentReference: !bictorisCallbackData.paymentReference,
                    },
                }, true, 'callback');
                return {
                    status: 'ERROR',
            ...
            let transaction;
            if (bictorisCallbackData.paymentReference) {
                transaction = await Transactions_entity_1.Transactions.findOne({
                    where: {
                        transactionId: bictorisCallbackData.paymentReference,
                        statut: typeorm_1.In([Enum_entity_1.StatusEnum.PENDING, Enum_entity_1.StatusEnum.PROCESSING]),
                        codeSousService: typeorm_1.In([
                            Enum_entity_1.SOUS_SERVICE_ENUM.WAVE_SN_API_CASH_OUT,
                            Enum_entity_1.SOUS_SERVICE_ENUM.ORANGE_SN_API_CASH_OUT,
                        ]),
                    },
                    relations: ['sousServices', 'parteners'],
                });
            }
            if (!transaction && bictorisCallbackData.merchantReference) {
                transaction = await Transactions_entity_1.Transactions.findOne({
                    where: {
                        externalTransactionId: bictorisCallbackData.merchantReference,
                        statut: typeorm_1.In([Enum_entity_1.StatusEnum.PENDING, Enum_entity_1.StatusEnum.PROCESSING]),
                        codeSousService: typeorm_1.In([
                            Enum_entity_1.SOUS_SERVICE_ENUM.WAVE_SN_API_CASH_OUT,
                            Enum_entity_1.SOUS_SERVICE_ENUM.ORANGE_SN_API_CASH_OUT,
            ...
            if (!transaction) {
                await this.helper.notifyAdmin('Bictoris callback: Transaction not found', Enum_entity_1.TypeEvenEnum.BICTORIS_CALLBACK_ERROR, {
                    bictorisCallbackData,
                    fromIp,
                    paymentReference: bictorisCallbackData.paymentReference,
                    merchantReference: bictorisCallbackData.merchantReference,
                }, false, 'callback');
                return {
                    status: 'ERROR',
            ...
            await this.helper.notifyAdmin('New Bictoris callback received', Enum_entity_1.TypeEvenEnum.BICTORIS_CALLBACK, {
                bictorisCallbackData,
                transactionId: transaction.transactionId,
                externalTransactionId: transaction.externalTransactionId,
                fromIp,
            ...
            const apiManagerService = await this.helper.getApiManagerInterface(transaction.codeSousService, null);
            if (!apiManagerService) {
                await this.helper.notifyAdmin('Bictoris callback: API manager service not configured', Enum_entity_1.TypeEvenEnum.BICTORIS_CALLBACK_ERROR, {
                    bictorisCallbackData,
                    fromIp,
                    codeSousService: transaction.codeSousService,
                }, true, 'callback');
                return {
                    status: 'ERROR',
                    message: 'API Service Manager not configured',
                };
            }
            const success = bictorisCallbackData.status === 'succeeded';
            if (success) {
                transaction.statut = Enum_entity_1.StatusEnum.SUCCESS;
                transaction.preStatut = Enum_entity_1.StatusEnum.SUCCESS;
                transaction.checkTransactionResponse = main_1.serializeData(Object.assign({
            ...
            else if (bictorisCallbackData.status == 'failed') {
                transaction.statut = Enum_entity_1.StatusEnum.FAILLED;
                transaction.preStatut = Enum_entity_1.StatusEnum.FAILLED;
                transaction.checkTransactionResponse = main_1.serializeData(Object.assign({
                    fromIp,
                }, bictorisCallbackData));
                transaction.errorMessage = `Bictoris payment ${bictorisCallbackData.status}: ${bictorisCallbackData.merchantReference}`;
            }
            else {
                transaction.checkTransactionResponse = main_1.serializeData(Object.assign({
                    fromIp,
            ...
            transaction.sousServiceTransactionId = bictorisCallbackData.id;
            await transaction.save();
            await apiManagerService.helper.setIsCallbackReadyValue(transaction, 0);
            apiManagerService.helper
                .updateApiBalance(apiManagerService, transaction.phonesId)
            ...
            if (success) {
                await apiManagerService.helper.handleSuccessTransactionCreditDebit(transaction);
                console.log('Bictoris callback processed successfully - SUCCESS');
                return {
                    status: 'OK',
                    transactionId: bictorisCallbackData.paymentReference,
                    message: 'OK_CALLBACK',
                ...
                await apiManagerService.helper.operationPartnerCancelTransaction(transaction);
                console.log('Bictoris callback processed successfully - FAILED');
                return {
                    status: 'ERROR',
                    transactionId: bictorisCallbackData.paymentReference,
                    message: 'FAILED_CALLBACK',
            ...
            console.error('Error processing Bictoris callback:', error);
            await this.helper.notifyAdmin('Error processing Bictoris callback', Enum_entity_1.TypeEvenEnum.BICTORIS_CALLBACK_ERROR, {
                bictorisCallbackData,
                fromIp,
                error: error.message,
                stack: error.stack,
...
__decorate([
    common_1.Post('callback/moneygram/intech_group/:mode'),
    __param(0, common_1.Param('mode')),
    __param(1, common_1.Body()),
    __param(2, common_1.Req()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, MoneyGramCallback_1.MoneyGramCallback, Object]),
    __metadata("design:returntype", Promise)
], ApiServiceController.prototype, "moneyGramCallback", null);
__decorate([
...
__decorate([
    request_mapping_decorator_1.All('callback/bictoris'),
    __param(0, common_1.Req()),
    __param(1, common_1.Body()),
    ...
    __metadata("design:returntype", Promise)
], ApiServiceController.prototype, "bictorisCallback", null);
__decorate([

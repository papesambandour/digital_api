...
class WizallApiProvider {
    ...
    async confirmBillSenelec(police, amount, numfacture, { wizallAgentPhoneNumber, wizallAgentPin }) {
        ...
        if (!find) {
            if (pendingBills.error) {
                return {
                    success: false,
                    message: 'Échec de la récupération des informations lors de la tentative de connexion avec Senelec. Veuillez réessayer.',
                };
            }
            else {
                return {
        ...
        console.log(pendingBills.pendingBills, find);
        if (parseInt(find.amount) !== amount) {
            return {
                success: false,
